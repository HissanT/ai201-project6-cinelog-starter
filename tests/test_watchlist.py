...
def test_add_to_watchlist_allows_same_film_for_different_users(app, sample_user, sample_film):
    """
    Deduplication should be scoped to a single user, not the whole film.
    """
    with app.app_context():
        ...
        add_to_watchlist(user_id=sample_user, film_id=sample_film)
        add_to_watchlist(user_id=second_user.id, film_id=sample_film)
...
def test_remove_from_watchlist_deletes_entry(app, sample_user, sample_film):
    """
    Removing a watchlist entry should delete the row and return True.
    """
    with app.app_context():
        add_to_watchlist(user_id=sample_user, film_id=sample_film)
